...
some method to make your best guess, an interval of plausible values, and to determine
...
* After getting the flu, around how many days is someone infectious?
...
The idea of coming up with one best guess at a population parameter is more
formally known as **point estimation**. An example of point estimation would be
using the sample mean to estimate a population mean. It is a single number 
computed from the sample data that is used to infer the value of the population 
parameter. However, as we have seen, the process of obtaining a sample is random,
...
within. We might consider this our *margin of error* for the point estimate. 
This process is known as **interval estimation**, and it is often more informative
than point estimation, making it a critical part of performing statistical inference.
...
Let's consider some desirable properties of a good interval. For 
example, it would be nice if our constructed interval had a high chance of 
containing the true population parameter. Intuitively, the larger the interval, 
the more likely that the true parameter falls within this range. However, if an 
interval is *too* large, it ceases to contain much useful information. If, when
planning what to wear this weekend, we reported that the temperature was supposed
to be somewhere between absolute zero and the heat found at the center of the 
sun, this would surely capture the actual temperature, but it would not 
be terribly useful. What we see here is a tension between two competing goals: 
we would like our intervals to be large enough to capture the true parameter 
value with high probability, yet small enough to give us a reasonable range of 
estimates of what this value might be. Fortunately, we have statistical tools 
available to do this - namely, the Central Limit Theorem.
...
and it gives us the parameters (mean and standard deviation) of said distribution.
In particular, it tell us that the distribution is symmetric around the true
population mean. Once we know the distribution of the sample mean, we can compute 
probabilities - and we can use this to determine the probability that the true 
mean is contained within some range of values. This is done by splitting the
normal curve into three parts, as shown in the figure below. The x-axis cutoffs 
$-z_{\alpha/2}$ and $z_{\alpha/2}$, divide the curve symmetrically, such that 
$1- \alpha$ probability lies in the middle and $\alpha/2$ probability lies in 
each tail. We call $1- \alpha$ the **coverage probability**, as it is the
probability covering the true mean. Just as $1 - \alpha$ represents the 
probability of covering the true value, the value of $\alpha$ represents the
probability *not* covering the true mean. The cutoff point $z_{\alpha/2}$ is 
...
The coverage probability and critical values depend on one variable - $\alpha$,
which is some value between 0 and 1. By decreasing $\alpha$, we trap more
probability in the center of the distribution and less in the tails. We can see
this in action by comparing two different alpha values: 0.05 and 0.2.
...
distributions as we know that sampling is a random process. 
