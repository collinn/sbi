...
Categorical and continuous data are summarized differently, and we'll explore a number of 
...
**Interval data: ** <em> Data in which scale is defined in terms of differences between observations; zero is arbitrary </em>

**Ratio data: ** <em> Data in which scale refers to real relationships between observations; zero is not arbitrary </em>
</div>
</div>
...
be interested in **proportions**. A proportion can be as simple as taking the total number of a single category observed, and stating it in terms of the total number of 
...
had brown hair." Proportions are also sometimes known as **relative frequencies**, because
they are relative to a specific number of observations. More commonly, we use
**percentages**, a special type of **proportions** -- where the quantity is considered per 100 observations. That is, 286 of 592 subjects becomes
...
By considering all observations as proportion per one hundred, we can quickly compare 
...
**Rate:** <em> When one quantity is divided by another, often (but not necessarily) with different units. A proportion is a type of rate in which the units are the same, though a rate such as "miles per hour" would not be a proportion. </em>
...
and dividing by the total. With $n$ observations, $x_1, x_2, ..., x_n$, 
...
consider an example in Table 2.4 where we collect $n = 10$ samples of salaries for University 
of Iowa employees:

```{r, echo = FALSE}
sals <- c("$31,176", "$50,879", "$34,619", "$103,000", "$36,549", "$130,000", "$37,876", "$144,600", "$48,962", "$5,075,000")
kable(matrix(sals, ncol = 2),  caption = "University of Iowa Salaries", padding = 2) |>
  kable_styling(full_width = FALSE, position = "float_right")
# kbl(matrix(sals, ncol = 2)) |> 
...
Measures that are less sensitive to outliers are called **robust** measures. The median is a robust estimator of the center of the data. 
...
them to differ? Here, we consider a collection of histograms showing us different "shapes" that the distributions of our data may take.


```{r, echo = FALSE, fig.height=5, fig.width = 7, fig.cap="Examples of modality and skew"}
...
<div class="definition"> &nbsp; </div>
<div class="text">
**Mean: ** <em> The average value, denoted $\bar{x}$ and computed as the sum of the observations divided by the number of obervations </em>

**Median: ** <em> The middle value or 50th percentile, the value such that half the observations lie below it and half above </em>
...
**Symmetric: ** <em> Characterization of a distribution with equal tails on both sides of the peak </em>
  
**Skewed right: ** <em> Characterization of a distribution with a large tail to the right of the peak </em>
   
**Skewed left: ** <em> Characterization of a distribution with a large tail to the left of the peak </em>
</div>
</div>
...
same mean or median without necessarily having the same shape.
...
the median and 50% are greater than it. $Q_1$ can be said to represent the median of the 
...
as an interval as $(Q_1, Q_3)$. For the New York temperature data, $Q_1 = 72$, $Q_3 = 85$. The IQR is therefore $85 - 72 = 13$ and tells us that 50% of the days
...
measure of the spread of the data. So, like the median, it enjoys the quality of being a robust measure of the data.

<!-- [[This paragraph was moved to the var/sd section]] -->
<!-- The IQR is not impacted by the presence of outliers, so it is considered a robust -->
...
<div class="definition"> &nbsp; </div>
<div class="text">
**Percentile: ** <em> A value, $V_p$ such that $p$\% of observations are smaller than $V_p$ and $1-p$\% of observations are larger than $V_p$ </em>

**Range: ** <em> The distance between the minimum and maximum values in a dataset </em>
...
The standard deviation, denoted $s$, is a function of the variance. 
...
of 89.6&deg;F^2^, a value that does not readily lend itself to interpretation. The standard deviation, on the other hand, takes the square root, putting
...
preferred as a measure of spread over the variance.
...
```{r}
kable(head(aq, 10), align = 'c') |> kable_styling(full_width = FALSE)
```
